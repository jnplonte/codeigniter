...
|
*/
$config['rest_realm'] = 'PCOMMUTE API';
...
$config['rest_login_table'] = 'api_logins';

$config['rest_login_user_column'] = 'username';
$config['rest_login_pass_column'] = 'password';
...
$result = mysqli_query($con, "select ".$config['rest_login_user_column'].", ".$config['rest_login_pass_column']." from ". $config['rest_login_table']. " where active=1");
while($row = mysqli_fetch_array($result)){
  $rest_valid_logins[$row[$config['rest_login_user_column']]] = $row[$config['rest_login_pass_column']];
}


$config['rest_valid_logins'] = $rest_valid_logins;
...
$config['rest_ip_whitelist_table'] = 'api_ip_whitelists';

$config['rest_ip_whitelist_ip_column'] = 'ip_address';

$rest_ip_whitelist = array();

$result = mysqli_query($con, "select ".$config['rest_ip_whitelist_ip_column']." from ". $config['rest_ip_whitelist_table']. " where active=1");
while($row = mysqli_fetch_array($result)){
  $rest_ip_whitelist[] = $row[$config['rest_ip_whitelist_ip_column']];
}

mysqli_close($con);

$config['rest_ip_whitelist'] = implode(", ",$rest_ip_whitelist);
...
|
*/
$config['rest_key_name'] = 'PCOMMUTE-API-KEY';
...
|--------------------------------------------------------------------------
|
| show the mother fucking execution time on api 
|
|	FALSE
|
*/
$config['show_exec_time'] 		= TRUE;
