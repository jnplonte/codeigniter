	...
	*/
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			// jnpl set default to 1 to prevent suspended api key
			$level = isset($this->methods[$controller_method]['level']) ? $this->methods[$controller_method]['level'] : 1;
	...
	public function response($data = array(), $http_code = null)
	{
		...
		else
		{
			...
			//jnpl added func
			if($this->config->item('show_exec_time')){
				$data['exec_time'] = get_exec_time($this->time_start);
			}
